...
public:
    ...
    bool _isAnnounceCC	 = sConfigMgr->GetBoolDefault("ChatCensure.Announce", true);
			...
			void CheckMessage(Player* player, std::string& msg, uint32 lang, Player* /*receiver*/, Group* /*group*/, Guild* /*guild*/, Channel* channel) 
			{
				...
				{
					msg = "";
					ChatHandler(player->GetSession()).PSendSysMessage("Palavra não permitida!");
					return;
				} 
			}
...
public:
    ...
    static bool HandleReloadCommand(ChatHandler* handler, char const* args)
    {
        ...
        ChatHandler(me->GetSession()).PSendSysMessage("Table chat_censure reloaded {} censured words in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
        return true;
    ...
    static bool HandleAddCommand(ChatHandler* handler, char const* args)
    {
        ...
        {	
			...
			ChatHandler(me->GetSession()).PSendSysMessage("Escreva a palavra a censurar");
            return false;
        }
        ...
        if (result)
        {
            ChatHandler(me->GetSession()).PSendSysMessage("Já existe a palavra: |cff4CFF00 {}|r", text.c_str());
        }
        else
        {
            CharacterDatabase.Query("INSERT INTO `chat_censure` (`text`) VALUES ('{}')", text.c_str());
            ChatHandler(me->GetSession()).PSendSysMessage("Censurada: |cff4CFF00 {}|r a la censura de chat. Recarge la tabla para activarla", text.c_str());
        }
        return true;
    }
    ...
    static bool HandleDeleteCommand(ChatHandler* handler, char const* args)
    {
        ...
        {	
			handler->SendSysMessage(LANG_IMPROPER_VALUE);
            handler->SetSentErrorMessage(true);
			ChatHandler(me->GetSession()).PSendSysMessage("Escreva a palavra pra censurar");
            return false;
        }
        ...
        if (!result)
        {
            ChatHandler(me->GetSession()).PSendSysMessage("Não se encontra a palavra : |cff4CFF00 {}|r na database", text.c_str());
        }
        else
        {
            CharacterDatabase.Query("DELETE FROM `chat_censure` WHERE `text` = '{}'", text.c_str());
            ChatHandler(me->GetSession()).PSendSysMessage("Censurada: |cff4CFF00 {}|r Recarregue a tabela para ativala", text.c_str());
        }
