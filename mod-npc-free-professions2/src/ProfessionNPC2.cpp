...
class CreatureScript_Professions2 : public CreatureScript
{
public:
    CreatureScript_Professions2() : CreatureScript("npc_teleporter") {}
    ...
    static void LearnAllRecipesInProfession(Player *player, SkillType skill)
    {
        if (HasFreeProfession(player, skill))
        {
            ...
            // Dá todas spells/recipes da profissão selecionada.
            if (SkillLineEntry const *SkillInfo = sSkillLineStore.LookupEntry(skill))
            {
                player->SetSkill(SkillInfo->id, player->GetSkillStep(SkillInfo->id), 150, 150);
...
void AddSC_Professions2()
{
    new CreatureScript_Professions2();
}
