...
public:
    ...
    static bool HandleBetterReloadItemsCommand(ChatHandler* handler)
    {
        if (args.empty())
            return false;
        ...
        for (std::string_view entryStr : Acore::Tokenize(args, ' ', false))
        {
            ...
            {
                LOG_INFO("Item (Entry: {}) has wrong Class value ({})", entry, itemTemplate->Class);
                itemTemplate->Class = ITEM_CLASS_MISC;
            }

            if (itemTemplate->SubClass >= MaxItemSubclassValues[itemTemplate->Class])
            {
                LOG_INFO("Item (Entry: {}) has wrong Subclass value ({}) for class {}", entry, itemTemplate->SubClass, itemTemplate->Class);
                itemTemplate->SubClass = 0;// exist for all item classes
            }

            if (itemTemplate->Quality >= MAX_ITEM_QUALITY)
            {
                LOG_INFO("Item (Entry: {}) has wrong Quality value ({})", entry, itemTemplate->Quality);
            ...
            if (itemTemplate->Flags2 & ITEM_FLAGS_EXTRA_HORDE_ONLY)
            {
                if (FactionEntry const* faction = sFactionStore.LookupEntry(HORDE))
                    if ((itemTemplate->AllowableRace & faction->BaseRepRaceMask[0]) == 0)
                        LOG_INFO("Item (Entry: {}) has value ({}) in `AllowableRace` races, not compatible with ITEM_FLAGS_EXTRA_HORDE_ONLY ({}) in Flags field, item cannot be equipped or used by these races.",
                            entry, itemTemplate->AllowableRace, ITEM_FLAGS_EXTRA_HORDE_ONLY);

                if (itemTemplate->Flags2 & ITEM_FLAGS_EXTRA_ALLIANCE_ONLY)
                    LOG_INFO("Item (Entry: {}) has value ({}) in `Flags2` flags (ITEM_FLAGS_EXTRA_ALLIANCE_ONLY) and ITEM_FLAGS_EXTRA_HORDE_ONLY ({}) in Flags field, this is a wrong combination.",
            ...
            else if (itemTemplate->Flags2 & ITEM_FLAGS_EXTRA_ALLIANCE_ONLY)
            {
                if (FactionEntry const* faction = sFactionStore.LookupEntry(ALLIANCE))
                    if ((itemTemplate->AllowableRace & faction->BaseRepRaceMask[0]) == 0)
                        LOG_INFO("Item (Entry: {}) has value ({}) in `AllowableRace` races, not compatible with ITEM_FLAGS_EXTRA_ALLIANCE_ONLY ({}) in Flags field, item cannot be equipped or used by these races.",
                            entry, itemTemplate->AllowableRace, ITEM_FLAGS_EXTRA_ALLIANCE_ONLY);
            }

            if (itemTemplate->BuyCount <= 0)
            {
                LOG_INFO("Item (Entry: {}) has wrong BuyCount value ({}), set to default(1).", entry, itemTemplate->BuyCount);
                itemTemplate->BuyCount = 1;
            }

            if (itemTemplate->InventoryType >= MAX_INVTYPE)
            {
                LOG_INFO("Item (Entry: {}) has wrong InventoryType value ({})", entry, itemTemplate->InventoryType);
                itemTemplate->InventoryType = INVTYPE_NON_EQUIP;
            }

            if (itemTemplate->RequiredSkill >= MAX_SKILL_TYPE)
            {
                LOG_INFO("Item (Entry: {}) has wrong RequiredSkill value ({})", entry, itemTemplate->RequiredSkill);
...

            {
                ...
                if (req)
                {
                    if (!(itemTemplate->AllowableClass & CLASSMASK_ALL_PLAYABLE))
                        LOG_INFO("Item (Entry: {}) does not have any playable classes ({}) in `AllowableClass` and can't be equipped or used.", entry, itemTemplate->AllowableClass);

                    if (!(itemTemplate->AllowableRace & RACEMASK_ALL_PLAYABLE))
                        LOG_INFO("Item (Entry: {}) does not have any playable races ({}) in `AllowableRace` and can't be equipped or used.", entry, itemTemplate->AllowableRace);
                }
            }

            if (itemTemplate->RequiredSpell && !sSpellMgr->GetSpellInfo(itemTemplate->RequiredSpell))
            {
                LOG_INFO("Item (Entry: {}) has a wrong (non-existing) spell in RequiredSpell ({})", entry, itemTemplate->RequiredSpell);
                itemTemplate->RequiredSpell = 0;
            }

            if (itemTemplate->RequiredReputationRank >= MAX_REPUTATION_RANK)
                LOG_INFO("Item (Entry: {}) has wrong reputation rank in RequiredReputationRank ({}), item can't be used.", entry, itemTemplate->RequiredReputationRank);

            if (itemTemplate->RequiredReputationFaction)
            {
                if (!sFactionStore.LookupEntry(itemTemplate->RequiredReputationFaction))
                {
                    LOG_INFO("Item (Entry: {}) has wrong (not existing) faction in RequiredReputationFaction ({})", entry, itemTemplate->RequiredReputationFaction);
                    itemTemplate->RequiredReputationFaction = 0;
                }

                if (itemTemplate->RequiredReputationRank == MIN_REPUTATION_RANK)
                    LOG_INFO("Item (Entry: {}) has min. reputation rank in RequiredReputationRank (0) but RequiredReputationFaction > 0, faction setting is useless.", entry);
            }

            if (itemTemplate->MaxCount < -1)
            {
                LOG_INFO("Item (Entry: {}) has too large negative in maxcount (%i), replace by value (-1) no storing limits.", entry, itemTemplate->MaxCount);
                itemTemplate->MaxCount = -1;
            }

            if (itemTemplate->Stackable == 0)
            {
                LOG_INFO("Item (Entry: {}) has wrong value in stackable (%i), replace by default 1.", entry, itemTemplate->Stackable);
                itemTemplate->Stackable = 1;
            }
            else if (itemTemplate->Stackable < -1)
            {
                LOG_INFO("Item (Entry: {}) has too large negative in stackable (%i), replace by value (-1) no stacking limits.", entry, itemTemplate->Stackable);
                itemTemplate->Stackable = -1;
            }

            if (itemTemplate->ContainerSlots > MAX_BAG_SIZE)
            {
                LOG_INFO("Item (Entry: {}) has too large value in ContainerSlots (%u), replace by hardcoded limit ({}).", entry, itemTemplate->ContainerSlots, MAX_BAG_SIZE);
                itemTemplate->ContainerSlots = MAX_BAG_SIZE;
            }

            if (itemTemplate->StatsCount > MAX_ITEM_PROTO_STATS)
            {
                LOG_INFO("Item (Entry: {}) has too large value in statscount (%u), replace by hardcoded limit ({}).", entry, itemTemplate->StatsCount, MAX_ITEM_PROTO_STATS);
            ...
            for (uint8 j = 0; j < itemTemplate->StatsCount; ++j)
            {
                ...
                {
                    LOG_INFO("Item (Entry: {}) has wrong (non-existing?) stat_type%d ({})", entry, j + 1, itemTemplate->ItemStat[j].ItemStatType);
                ...
                case ITEM_MOD_SPELL_DAMAGE_DONE:
                    LOG_INFO("Item (Entry: {}) has deprecated stat_type%d ({})", entry, j + 1, itemTemplate->ItemStat[j].ItemStatType);
                    break;
                default:
                    break;
                }
            ...
            for (uint8 j = 0; j < MAX_ITEM_PROTO_DAMAGES; ++j)
            {
                ...
                {
                    LOG_INFO("Item (Entry: {}) has wrong dmg_type%d ({})", entry, j + 1, itemTemplate->Damage[j].DamageType);
            ...
            if ((itemTemplate->Spells[0].SpellId == 483) || (itemTemplate->Spells[0].SpellId == 55884))
            {
                ...
                {
                    LOG_INFO("Item (Entry: {}) has wrong item spell trigger value in spelltrigger_%d ({}) for special learning format", entry, 0 + 1, itemTemplate->Spells[0].SpellTrigger);
                ...
                {
                    LOG_INFO("Item (Entry: {}) has wrong item spell trigger value in spelltrigger_%d ({}) for special learning format.", entry, 1 + 1, itemTemplate->Spells[1].SpellTrigger);
                    itemTemplate->Spells[0].SpellId = 0;
                    itemTemplate->Spells[1].SpellId = 0;
                    itemTemplate->Spells[1].SpellTrigger = ITEM_SPELLTRIGGER_ON_USE;
                }
                ...
                {
                    LOG_INFO("Item (Entry: {}) does not have an expected spell in spellid_%d in special learning format.", entry, 1 + 1);
                    itemTemplate->Spells[0].SpellId = 0;
                    itemTemplate->Spells[1].SpellTrigger = ITEM_SPELLTRIGGER_ON_USE;
                }
                else if (itemTemplate->Spells[1].SpellId != -1)
                {
                    ...
                    {
                        LOG_INFO("Item (Entry: {}) has wrong (not existing) spell in spellid_%d ({})", entry, 1 + 1, itemTemplate->Spells[1].SpellId);
                        itemTemplate->Spells[0].SpellId = 0;
                        itemTemplate->Spells[1].SpellId = 0;
                        itemTemplate->Spells[1].SpellTrigger = ITEM_SPELLTRIGGER_ON_USE;
                    }
                    ...
                    {
                        LOG_INFO("Item (Entry: {}) has broken spell in spellid_%d ({})", entry, 1 + 1, itemTemplate->Spells[1].SpellId);
                        itemTemplate->Spells[0].SpellId = 0;
                        itemTemplate->Spells[1].SpellId = 0;
                        itemTemplate->Spells[1].SpellTrigger = ITEM_SPELLTRIGGER_ON_USE;
                    }
                ...
                for (uint8 j = 2; j < MAX_ITEM_PROTO_SPELLS; ++j)
                {
                    ...
                    {
                        LOG_INFO("Item (Entry: {}) has wrong item spell trigger value in spelltrigger_%d ({})", entry, j + 1, itemTemplate->Spells[j].SpellTrigger);
                    ...
                    {
                        LOG_INFO("Item (Entry: {}) has wrong spell in spellid_%d (%d) for learning special format", entry, j + 1, itemTemplate->Spells[j].SpellId);
                        itemTemplate->Spells[j].SpellId = 0;
                    }
                }
            }
            ...
            else
            {
                for (uint8 j = 0; j < MAX_ITEM_PROTO_SPELLS; ++j)
                {
                    ...
                    if (itemTemplate->Spells[j].SpellId && itemTemplate->Spells[j].SpellId != -1)
                    {
                        ...
                        {
                            LOG_INFO("Item (Entry: {}) has wrong (not existing) spell in spellid_%d ({})", entry, j + 1, itemTemplate->Spells[j].SpellId);
                            itemTemplate->Spells[j].SpellId = 0;
                        }
                        // allowed only in special format
                        else if ((itemTemplate->Spells[j].SpellId == 483) || (itemTemplate->Spells[j].SpellId == 55884))
                        {
                            LOG_INFO("Item (Entry: {}) has broken spell in spellid_%d ({})", entry, j + 1, itemTemplate->Spells[j].SpellId);
                            itemTemplate->Spells[j].SpellId = 0;
                        }
                    }
                }
            ...
            if (itemTemplate->Bonding >= MAX_BIND_TYPE)
                LOG_INFO("Item (Entry: {}) has wrong Bonding value ({})", entry, itemTemplate->Bonding);

            if (itemTemplate->PageText && !sObjectMgr->GetPageText(itemTemplate->PageText))
                LOG_INFO("Item (Entry: {}) has non existing first page (Id:{})", entry, itemTemplate->PageText);

            if (itemTemplate->LockID && !sLockStore.LookupEntry(itemTemplate->LockID))
                LOG_INFO("Item (Entry: {}) has wrong LockID ({})", entry, itemTemplate->LockID);

            if (itemTemplate->Sheath >= MAX_SHEATHETYPE)
            {
                sLOG_INFO("Item (Entry: {}) has wrong Sheath ({})", entry, itemTemplate->Sheath);
            ...
            if (itemTemplate->RandomProperty)
            {
                ...
                {
                    LOG_INFO("Item (Entry: {}) has unknown (wrong or not listed in `item_enchantment_template`) RandomProperty ({})", entry, itemTemplate->RandomProperty);
                    itemTemplate->RandomProperty = 0;
                }
            }
            ...
            {
                sLOG_INFO("Item (Entry: {}) has wrong RandomSuffix ({})", entry, itemTemplate->RandomSuffix);
                itemTemplate->RandomSuffix = 0;
            }

            if (itemTemplate->ItemSet && !sItemSetStore.LookupEntry(itemTemplate->ItemSet))
            {
                LOG_INFO("Item (Entry: {}) have wrong ItemSet ({})", entry, itemTemplate->ItemSet);
                itemTemplate->ItemSet = 0;
            }

            if (itemTemplate->Area && !sAreaTableStore.LookupEntry(itemTemplate->Area))
                LOG_INFO("Item (Entry: {}) has wrong Area ({})", entry, itemTemplate->Area);

            if (itemTemplate->Map && !sMapStore.LookupEntry(itemTemplate->Map))
                LOG_INFO("Item (Entry: {}) has wrong Map ({})", entry, itemTemplate->Map);
            ...
            if (itemTemplate->BagFamily)
            {
                ...
                for (uint32 j = 0; j < sizeof(itemTemplate->BagFamily) * 8; ++j)
                {
                    ...
                    if (!bf)
                    {
                        LOG_INFO("Item (Entry: {}) has bag family bit set not listed in ItemBagFamily.dbc, remove bit", entry);
                    ...
                    if (BAG_FAMILY_MASK_CURRENCY_TOKENS & mask)
                    {
                        ...
                        if (!ctEntry)
                        {
                            LOG_INFO("Item (Entry: {}) has currency bag family bit set in BagFamily but not listed in CurrencyTypes.dbc, remove bit", entry);
                            itemTemplate->BagFamily &= ~mask;
                        }
                    }
                }
            ...
            if (itemTemplate->TotemCategory && !sTotemCategoryStore.LookupEntry(itemTemplate->TotemCategory))
                LOG_INFO("Item (Entry: {}) has wrong TotemCategory ({})", entry, itemTemplate->TotemCategory);
            ...
            for (uint8 j = 0; j < MAX_ITEM_PROTO_SOCKETS; ++j)
            {
                ...
                {
                    LOG_INFO("Item (Entry: {}) has wrong socketColor_%d ({})", entry, j + 1, itemTemplate->Socket[j].Color);
                    itemTemplate->Socket[j].Color = 0;
                }
            }

            if (itemTemplate->GemProperties && !sGemPropertiesStore.LookupEntry(itemTemplate->GemProperties))
                LOG_INFO("Item (Entry: {}) has wrong GemProperties ({})", entry, itemTemplate->GemProperties);

            if (itemTemplate->FoodType >= MAX_PET_DIET)
            {
                LOG_INFO("Item (Entry: {}) has wrong FoodType value ({})", entry, itemTemplate->FoodType);
                itemTemplate->FoodType = 0;
            }

            if (itemTemplate->ItemLimitCategory && !sItemLimitCategoryStore.LookupEntry(itemTemplate->ItemLimitCategory))
            {
                LOG_INFO("Item (Entry: {}) has wrong LimitCategory value ({})", entry, itemTemplate->ItemLimitCategory);
                itemTemplate->ItemLimitCategory = 0;
            }

            if (itemTemplate->HolidayId && !sHolidaysStore.LookupEntry(itemTemplate->HolidayId))
            {
                LOG_INFO("Item (Entry: {}) has wrong HolidayId value ({})", entry, itemTemplate->HolidayId);
                itemTemplate->HolidayId = 0;
            }

            if (itemTemplate->FlagsCu & ITEM_FLAGS_CU_DURATION_REAL_TIME && !itemTemplate->Duration)
            {
                LOG_INFO("Item (Entry {}) has flag ITEM_FLAGS_CU_DURATION_REAL_TIME but it does not have duration limit", entry);
