...
class CreatureScript_Teleporter : public CreatureScript
{
public:
    CreatureScript_Teleporter() : CreatureScript("npc_teleporter") {}
    ...
    static void LearnAllRecipesInProfession(Player *player, SkillType skill)
    {
        if (HasFreeProfession(player, skill))
        {
            ...
            {
                player->SetSkill(SkillInfo->id, player->GetSkillStep(SkillInfo->id), 1, 150);
