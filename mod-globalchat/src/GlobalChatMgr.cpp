...
std::string GlobalChatMgr::GetChatPrefix()
{
    ...
    if (!ChatName.empty())
    {
        ...
        if (JoinChannel)
        {
            ...
            chatPrefix << "c " << "Global"; // acho que se mudar o ChatName para "Global", vai aparecer "Global", mas o nome do channel vai ser algum numero (para nao ter problema com letra maiuscula)
        }
        else
        {
            chatPrefix << "s .global ";
        ...
        chatPrefix << "[" << "Global" << "]|h"; //aqui tbm se pa
    }
...
void GlobalChatMgr::PlayerJoinCommand(ChatHandler* handler)
{
    ...
    handler->PSendSysMessage("Voce entrou no GlobalChat. Digite .g + texto para mandar mensagem no Chat.");
    handler->PSendSysMessage("É possivel mandar mensagem atraves do channel Global tambem, para entrar digite /join .");
