...
enum MorphSummonSpells
{
    ...
    RAISE_DEAD = 52150,
    //SUMMON_WATER_ELEMENTAL = 70907 // Sem Glyph - reseta o display quando sumona denovo, provavelmente é um problema core-side (deveria salvar o auto toggle e talvez fica salvo o pet na tabela tbm, dai daria para salvar o Display sem glyph)
    SUMMON_WATER_ELEMENTAL = 70908 // Com Glyph
};
...
public:
    ...
    void OnAfterGuardianInitStatsForLevel(Player* player, Guardian* guardian) override
    {
        if (Pet* pet = guardian->ToPet())
        {
            ...
            // Voidwalker pets sumonados/resumonados
            else if (pet->GetUInt32Value(UNIT_CREATED_BY_SPELL) == SUMMON_VOIDWALKER)
            {
                float nativeScale = pet->GetNativeObjectScale();
                // VoidWraithSkin_Purple - Scale 2
                if (pet->GetDisplayId() == 18050)
                {
                    CreatureDisplayInfoEntry const* displayInfo = sCreatureDisplayInfoStore.LookupEntry(pet->GetDisplayId());
                    pet->SetObjectScale(2.30f / displayInfo->scale);
                }
                // VoidWraithSkin_Red - Scale 1,2
                else if (pet->GetDisplayId() == 18919)
                {
                    CreatureDisplayInfoEntry const* displayInfo = sCreatureDisplayInfoStore.LookupEntry(pet->GetDisplayId());
                    pet->SetObjectScale(1.40f / displayInfo->scale);
                }
                // VoidWraithSkin_Blue - Scale 1
                else if (pet->GetDisplayId() == 18069)
                {
                    CreatureDisplayInfoEntry const* displayInfo = sCreatureDisplayInfoStore.LookupEntry(pet->GetDisplayId());
                    pet->SetObjectScale(1.15f / displayInfo->scale);
                }
                else
                {
                    pet->SetObjectScale(nativeScale);
                }
            }
            // Felhunter pets sumonados/resumonados
            else if (pet->GetUInt32Value(UNIT_CREATED_BY_SPELL) == SUMMON_FELHUNTER)
            {
                float nativeScale = pet->GetNativeObjectScale();
                // Observer
                if (pet->GetDisplayId() == 19579)
                {
                    CreatureDisplayInfoEntry const* displayInfo = sCreatureDisplayInfoStore.LookupEntry(pet->GetDisplayId());
                    pet->SetObjectScale(0.40f / displayInfo->scale);
                }
                else
                {
                    pet->SetObjectScale(nativeScale);
            ...
            // DK Ghoul pets sumonados/resumonados
            else if (pet->GetUInt32Value(UNIT_CREATED_BY_SPELL) == RAISE_DEAD)
            {
                float nativeScale = pet->GetNativeObjectScale();
                // Superzombies (Cataclysm dark transformation)
                if (pet->GetDisplayId() == 22122 || pet->GetDisplayId() == 22123 || pet->GetDisplayId() == 22124 || pet->GetDisplayId() == 22125)
                {
                    CreatureDisplayInfoEntry const* displayInfo = sCreatureDisplayInfoStore.LookupEntry(pet->GetDisplayId());
                    pet->SetObjectScale(0.85f / displayInfo->scale);
                }
            ...
            if (sender >= startPage && sender < maxPage)
            {
                if (petOrMinion->GetUInt32Value(UNIT_CREATED_BY_SPELL) == spell)
                {
                    ...
                    if (polymorphPet)
                    {
                        ...
                        // muda o display/size do pet sumonado atual OnGossip
                        if (petOrMinion->GetUInt32Value(UNIT_CREATED_BY_SPELL) == SUMMON_VOIDWALKER)
                        {
                            float nativeScale = petOrMinion->GetNativeObjectScale();
                            // VoidWraithSkin_Purple - Scale 2
                            if (petOrMinion->GetDisplayId() == 18050)
                            {
                                CreatureDisplayInfoEntry const* displayInfo = sCreatureDisplayInfoStore.LookupEntry(petOrMinion->GetDisplayId());
                                petOrMinion->SetObjectScale(2.30f / displayInfo->scale);
                            }
                            // VoidWraithSkin_Red - Scale 1,2
                            else if (petOrMinion->GetDisplayId() == 18919)
                            {
                                CreatureDisplayInfoEntry const* displayInfo = sCreatureDisplayInfoStore.LookupEntry(petOrMinion->GetDisplayId());
                                petOrMinion->SetObjectScale(1.40f / displayInfo->scale);
                            }
                            // VoidWraithSkin_Blue - Scale 1
                            else if (petOrMinion->GetDisplayId() == 18069)
                            {
                                CreatureDisplayInfoEntry const* displayInfo = sCreatureDisplayInfoStore.LookupEntry(petOrMinion->GetDisplayId());
                                petOrMinion->SetObjectScale(1.15f / displayInfo->scale);
                            }
                            else
                            {
                                petOrMinion->SetObjectScale(nativeScale);
                            }
                        }
                        // Felhunter pets modificados pelo Gossip
                        if (petOrMinion->GetUInt32Value(UNIT_CREATED_BY_SPELL) == SUMMON_FELHUNTER)
                        {
                            float nativeScale = petOrMinion->GetNativeObjectScale();
                            // Observer
                            if (petOrMinion->GetDisplayId() == 19579)
                            {
                                CreatureDisplayInfoEntry const* displayInfo = sCreatureDisplayInfoStore.LookupEntry(petOrMinion->GetDisplayId());
                                petOrMinion->SetObjectScale(0.40f / displayInfo->scale);
                            }
                            else
                            {
                                petOrMinion->SetObjectScale(nativeScale);
                        ...
                        // DK Ghoul pets modificados pelo Gossip
                        if (petOrMinion->GetUInt32Value(UNIT_CREATED_BY_SPELL) == RAISE_DEAD)
                        {
                            float nativeScale = petOrMinion->GetNativeObjectScale();
                            // Superzombies (Cataclysm dark transformation)
                            if (petOrMinion->GetDisplayId() == 22122 || petOrMinion->GetDisplayId() == 22123 || petOrMinion->GetDisplayId() == 22124 || petOrMinion->GetDisplayId() == 22125)
                            {
                                CreatureDisplayInfoEntry const* displayInfo = sCreatureDisplayInfoStore.LookupEntry(petOrMinion->GetDisplayId());
                                petOrMinion->SetObjectScale(0.85f / displayInfo->scale);
                            }
                        ...
                        Aura* submergeAura = petOrMinion->AddAura(SUBMERGE, pet);
                        Aura* visualAura = petOrMinion->AddAura(SHADOW_SUMMON_VISUAL, pet);

                        if (submergeAura && visualAura) {
                            submergeAura->SetDuration(300);
                            visualAura->SetDuration(300);
                        }
